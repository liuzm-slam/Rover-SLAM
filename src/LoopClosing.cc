...
LoopClosing::LoopClosing(Atlas *pAtlas, KeyFrameDatabase *pDB, SPVocabulary *pVoc, const bool bFixScale, const bool bActiveLC):
    ...
    mpKeyFrameDB(pDB), mpSPVocabulary(pVoc), mpMatchedKF(NULL), mLastLoopKFid(0), mbRunningGBA(false), mbFinishedGBA(true),
            ...
            if(bFindedRegion)
            {
                ...
                if(mbLoopDetected)
                {
                    ...
                    if(mpCurrentKF->GetMap()->IsInertial())
                    {
                        ...
                        if (fabs(phi(0))<0.08f && fabs(phi(1))<0.08f && fabs(phi(2))<0.349f)
                        {
                            // 如果是imu模式
                            if(mpCurrentKF->GetMap()->IsInertial())
                            {
...
bool LoopClosing::NewDetectCommonRegions()
{
    ...
    if(mpLastMap->IsInertial() && !mpLastMap->GetIniertialBA2())
    {
        ...
        //mpKeyFrameDB->add_sp(mpCurrentKF);
    ...
    {
        cout << "huihuanshixuyanzhenng!" << endl;
        bCheckSpatial = true;
        ...
        cout<<"bCommonRegion: "<<bCommonRegion<<endl;
        // 如果找到共同区域(时序验证成功一次)
        if(bCommonRegion)
        {
            //标记时序检验成功一次
    ...
    if(!bMergeDetectedInKF || !bLoopDetectedInKF)
    {
        ...
        mpKeyFrameDB->DetectNBestCandidates_sp(mpCurrentKF, vpLoopBowCand, vpMergeBowCand,4);
        //cout<<"Search in BoW success!"<<endl;
#ifdef REGISTER_TIMES
    ...
    //cout<<"kongjianshixuyanzheng!"<<endl;
    if(!bLoopDetectedInKF && !vpLoopBowCand.empty())
    {
        ...
        mbLoopDetected = DetectCommonRegionsFromBoW_sp(vpLoopBowCand, mpLoopMatchedKF, mpLoopLastCurrentKF, mg2oLoopSlw, mnLoopNumCoincidences, mvpLoopMPs, mvpLoopMatchedMPs);
        //cout<<"mbLoopDetected: "<<mbLoopDetected<<endl;
        //cout<<"vpLoopBowCand11: "<<vpLoopBowCand.size()<<endl;
    }   
    // Merge candidates
...
bool LoopClosing::DetectCommonRegionsFromBoW_sp(
    std::vector<KeyFrame*> &vpBowCand, KeyFrame* &pMatchedKF2, KeyFrame* &pLastCurrentKF, g2o::Sim3 &g2oScw,
    int &nNumCoincidences, std::vector<MapPoint*> &vpMPs, std::vector<MapPoint*> &vpMatchedMPs)
{
    // 一些后面会使用的阀值
    ...
    for(KeyFrame* pKFi : vpBowCand)
    {
        ...
        for(int j=0; j<vpCovKFi.size(); ++j)
        {
            ...
            {
                bAbortByNearKF = true;
                //cout<<"frame in windows is current frame's covisible frame!"<<endl;
                break;
            }
        }
        if(bAbortByNearKF)
        ...
        int num = mspmatcher.SearchByBoWSP(mpCurrentKF, vpCovKFi[j], vvpMatchedMPs[j], vmatches);
        ...
        for(int k=0; k < vvpMatchedMPs[j].size(); ++k)
        {
            ...
            if(!pMPi_j || pMPi_j->isBad())
                 continue;
            // 窗口内不同关键帧与当前关键帧可能看到相同的3D点, 利用辅助容器避免重复添加
            if(spMatchedMPi.find(pMPi_j) == spMatchedMPi.end())//当该地图点不在set容器中时
            {
                // 利用辅助容器记录添加过的点
                spMatchedMPi.insert(pMPi_j);
                // 统计窗口内有多少地图点能在当前关键中找到匹配
        ...
        //     vector<cv::DMatch> vmatches;
        //     //int num = mspmatcher.SearchByBoW(mpCurrentKF, vpCovKFi[j], vvpMatchedMPs[j], vmatches);
        //     int num = mspmatcher.SearchByBoWSP(mpCurrentKF, vpCovKFi[j], vvpMatchedMPs[j], vmatches);
        //     // cv::Mat matched_img;
        //     // cv::drawMatches(mpCurrentKF->mImg,mpCurrentKF->mvKeys,vpCovKFi[j]->mImg,vpCovKFi[j]->mvKeys,vmatches,matched_img);
        //     // cv::imshow("Matched Features", matched_img);
        //     // cv::waitKey(200);
        //     // cv::Mat combined_image;
        //     // cv::hconcat(mpCurrentKF->mImg, vpCovKFi[j]->mImg, combined_image);
        //     // cv::imshow("并排显示", combined_image);
        //     // cv::waitKey(100);
        //     std::cout<<"num: "<<num<<" ";
        //     //int num = matcherBoW.SearchByBoW(mpCurrentKF, vpCovKFi[j], vvpMatchedMPs[j]);
        //     if (num > nMostBoWNumMatches)
        ...
        if(numBoWMatches >= nBoWMatches) // TODO pick a good threshold
        {
            ...
            // Sim3Solver Ransac 置信度0.99，至少20个inliers 最多300次迭
            solver.SetRansacParameters(0.97, nBoWInliers, 300); // at least 15 inliers
            ...
            while(!bConverge && !bNoMore)
            {
                ...
                //cout<<"BoW guess: Solver achieve " + to_string(nInliers) + " geometrical inliers among " + to_string(nBoWInliers) + " BoW matches"<<endl;
            }
            ...
            {
                //std::cout << "Check BoW: SolverSim3 converged" << std::endl;
                //cout<<pKFi->mImg<<mpCurrentKF->mImg<<endl;

                //  
                // cv::Mat combined;
                // cv::hconcat(pKFi->mImg, mpCurrentKF->mImg, combined);
                // cv::imshow("Combined Image", combined);
                // cout<<"pKFi->mnId: "<< pKFi->mnId<<" "<<mpCurrentKF->mnId<<endl;
                // cv::waitKey(200);
                ...
                if(numProjMatches >= nProjMatches)
                {
                    ...
                    // pKFi是候选关键帧
                    int numOptMatches = Optimizer::OptimizeSim3(mpCurrentKF, pKFi, vpMatchedMP, gScm, 10, mbFixScale, mHessian7x7, true);
                    cout<<"numOptMatches: "<<numOptMatches<<endl;
                    // 3.3.3 如果内点足够多,用更小的半径搜索匹配点,并且再次进行优化(p.s.这里与论文不符,并没有再次优化)
                    if(numOptMatches >= nSim3Inliers)
                    {
                        // 前面已经声明了这些变量了,无需再次声明
                        ...
                        // 5 : 半径的增益系数(对比之前下降了)---> 更小的半径, 1.0 , hamming distance 的阀值增益系数---> 允许更小的距离
                       // int numProjOptMatches = matcher.SearchByProjection(mpCurrentKF, mScw, vpMapPoints, vpMatchedMP, 5, 1.0);
                        int numProjOptMatches = mspmatcher.SearchByProjection(mpCurrentKF, mScw, vpMapPoints, vpMatchedMP, 5, 1.0);
                        cout<<"numProjOptMatches: "<<numProjMatches<<endl;
                        // 当新的投影得到的内点数量大于nProjOptMatches=80时
                        if(numProjOptMatches >= nProjOptMatches)
                        {
                            //cout<<"LC1"<<endl;
                            ...
                            while(nNumKFs < 3 && j<vpCurrentCovKFs.size())
                            {
                                ...
                                vector<MapPoint*> vpMatchedMPs_j;
                                // 4.2 几何校验函数, 这个函数里面其实是个searchByProjection : 通过之前计算的位姿转换地图点并通过投影搜索匹配点, 若大于一定数目的任务成功验证一次
                                bool bValid = DetectCommonRegionsFromLastKF(pKFj,pMostBoWMatchesKF, gSjw,numProjMatches_j, vpMapPoints, vpMatchedMPs_j);//该函数里有匹配函数，需要修改
    ...
    if(nBestMatchesReproj > 0)
    {
        ...
        vpMatchedMPs = vpBestMatchedMapPoints;
        // 如果有三个成功验证则return ture
        cout<<"nNumCoincidences: "<<nNumCoincidences<<endl;
        return nNumCoincidences >= 3;
    }
    else
    {
    ...
    std::vector<MapPoint*> &vpMPs, std::vector<MapPoint*> &vpMatchedMPs)
{
    ...
    cout<<"LC5  nNumProjMatches: "<<nNumProjMatches<<endl;
    return false;
}
    ...
    vector<MapPoint*> &vpMatchedMapPoints)
{
    ...
    int num_matches = mspmatcher.SearchByProjection(pCurrentKF, mScw, vpMapPoints, vpMatchedMapPoints, 3, 1.5);
    return num_matches;
}
