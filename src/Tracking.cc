...
void Tracking::newParameterLoader(Settings *settings) {
    ...
    //std::shared_ptr<SuperPoint> _superpoint = std::make_shared<SuperPoint>();
    // _superpoint->super_point_config_.engine_file="/home/xiao/catkin_ws/src/AirVO/output/superpoint_v1_sim_int32.engine";
    // _superpoint->super_point_config_.max_keypoints = 400;
    // _superpoint->super_point_config_.keypoint_threshold = 0.0040000000000000001;
    // _superpoint->super_point_config_.remove_borders = 4;
    // _superpoint->super_point_config_.dla_core = -1;
...
bool Tracking::ParseORBParamFile(cv::FileStorage &fSettings)
{
    ...
    if(mSensor==System::STEREO || mSensor==System::IMU_STEREO)
        //mpExtractorRight = new ORBextractor(nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST);
        mpExtractorRight = new SPextractor(5*nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST);
    if(mSensor==System::MONOCULAR || mSensor==System::IMU_MONOCULAR)
        //mpIniORBextractor = new ORBextractor(5*nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST);
        mpIniExtractor = new SPextractor(5*nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST);
...
Sophus::SE3f Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename)
{
    ...
    if (mSensor == System::STEREO && !mpCamera2)
        mCurrentFrame = Frame(
            ...
            imGrayRight,            // 右目图像
            timestamp,              // 时间戳
            mpExtractorLeft,     // 左目特征提取器
            mpExtractorRight,    // 右目特征提取器
            mpSPVocabulary,        // 字典
            mK,                     // 内参矩阵
            mDistCoef,              // 去畸变参数
            mbf,                    // 基线长度
...
Sophus::SE3f Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, string filename)
{
    ...
    // for (const cv::KeyPoint& point : mCurrentFrame.mvKeys) {
    //     cv::circle(mCurrentFrame.imgLeft ,point.pt,2,cv::Scalar(0, 0, 255), 0.5);
    // }
    // cv::imshow("Feature Points", mCurrentFrame.imgLeft);
    // cv::waitKey(0);
    // Step 3 ：跟踪
    Track();
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                ...
                if((!mbVelocity && !pCurrentMap->isImuInitialized()) || mCurrentFrame.mnId<mnLastRelocFrameId+2)
                {
                    ...
                    if(bOK)
                        cout<<"trackReferenceKeyFrame success!"<<endl;
                }
                else
                {
            ...
            else  // 跟踪不正常按照下面处理
            {
                ...
                if (mState == RECENTLY_LOST)
                {
                    ...
                    else
                    {
                        ...
                        if(mCurrentFrame.mTimeStamp-mTimeStampLost>2.0f && !bOK)//3.0f
                        {
                            // 纯视觉模式下重定位失败，状态为LOST
                            mState = LOST;
                            Verbose::PrintMess("Track Lost...", Verbose::VERBOSITY_NORMAL);
        ...
        if(!mbOnlyTracking)
        {
            ...
            {
                //cout<<"track local map success!"<<endl;
            }
        }
        else
        {
...
void Tracking::MonocularInitialization()
{
    ...
    if(!mbReadyToInitializate)
    {
        ...
        if(mCurrentFrame.mvKeys.size()>100)
        {
            ...
            // 用当前帧更新上一帧
            mLastFrame = Frame(mCurrentFrame);
            cout<<"!mbReadyToInitializate"<<endl;
    ...
    else  // 第二帧来了
    {
        ...
        if ((mLastFrame.mnId-mInitialFrame.mnId > 30)||((int)mCurrentFrame.mvKeys.size()<=100)||((mSensor == System::IMU_MONOCULAR)&&(mLastFrame.mTimeStamp-mInitialFrame.mTimeStamp>1.0)))
        {
        ...
        int nmatches_sp = mspmatcher.SearchForInitialization(mInitialFrame, mCurrentFrame, mvbPrevMatched, mvIniMatches);
        
        // if(mi%100 == 0){
        //     std::cout<<"frame id:"<<mi<<std::endl;
        //     mspmatcher.plotspmatch(mInitialFrame.imgLeft, mCurrentFrame.imgLeft, mInitialFrame.mvKeys, mCurrentFrame.mvKeys, mvIniMatches);
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    //SPmatcher matcher_sp(0.9);
    vector<MapPoint*> vpMapPointMatches;

    // Step 2：通过词袋BoW加速当前帧与参考帧之间的特征点匹配
    //int nmatches = matcher.SearchByBoW(mpReferenceKF,mCurrentFrame,vpMapPointMatches);
    int nmatches_sp = mspmatcher.SearchByBoWSP(mpReferenceKF, mCurrentFrame, vpMapPointMatches);
    // 匹配数目小于15，认为跟踪失败
    if(nmatches_sp<15)
    {
...
bool Tracking::TrackWithMotionModel()
{
    ...
    int nmatches_sp = mspmatcher.SearchBySP(mCurrentFrame, mLastFrame);
    //this->mpExtractorLeft->lastmatchnum = nmatches_sp;
    //cout<<"lastmatchnum : "<<this->mpExtractorLeft->lastmatchnum<<endl;
    //int nmatches_sp = mspmatcher.SearchByProjection(mCurrentFrame, mLastFrame,th, mSensor==System::MONOCULAR || mSensor==System::IMU_MONOCULAR);
    int first_nmatches = nmatches_sp;
    ...
    if(nmatches_sp<20)
    {
        ...
        fill(mCurrentFrame.mvpMapPoints.begin(),mCurrentFrame.mvpMapPoints.end(),static_cast<MapPoint*>(NULL));
        // cv::Mat combined;
        // cv::vconcat(mCurrentFrame.imgLeft, mLastFrame.imgLeft, combined);
...
bool Tracking::TrackLocalMap()
{
    ...
    if (mSensor == System::IMU_MONOCULAR)
    {
        if((mnMatchesInliers<15 && mpAtlas->isImuInitialized())||(mnMatchesInliers<10 && !mpAtlas->isImuInitialized()))
        {
            cout<<"aux1: "<< aux1 << "  aux2:  " <<aux2<<endl;
            cout<<"(mnMatchesInliers<10  "<< mnMatchesInliers<<endl;
            return false;
        }
        else
            return true;
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    if((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD) && !mpAtlas->GetCurrentMap()->isImuInitialized())
    {
        ...
        //cout<<"mpLastKeyFrame.mTimeStamp:"<<mpLastKeyFrame->mTimeStamp<<endl;    
        // 如果是IMU模式，当前帧距离上一关键帧时间戳超过0.25s，则说明需要插入关键帧，不再进行后续判断
        if (mSensor == System::IMU_MONOCULAR && (mCurrentFrame.mTimeStamp-mpLastKeyFrame->mTimeStamp)>=0.1)//0.2
        {
            return true;
        }
    ...
    if(mSensor==System::IMU_MONOCULAR)
    {
        if(mnMatchesInliers>350) // Points tracked from the local map
            thRefRatio = 0.75f; //0.75f;  5.0f
        else
            thRefRatio = 0.90f; //0.90f; 5.2f
    }
    ...
    //cout<<"mnMatchesInliers: "<<mnMatchesInliers<<"nRefMatches: "<<nRefMatches<<" nRefMatches*thRefRatio: "<<nRefMatches*thRefRatio<<endl;
    ...
    bool c4 = false;
    if ((((mnMatchesInliers<200) && (mnMatchesInliers>15)) || mState==RECENTLY_LOST) && (mSensor == System::IMU_MONOCULAR)) // MODIFICATION_2, originally ((((mnMatchesInliers<75) && (mnMatchesInliers>15)) || mState==RECENTLY_LOST) && ((mSensor == System::IMU_MONOCULAR)))
        c4=true;
    else
        c4=false;
    //std::cout << "NeedNewKF: c1a=" << c1a << "; c1b=" << c1b << "; c1c=" << c1c << "; c2=" << c2 <<"; c3="<<c3<<";c4 ="<<c4<<std::endl;   500
    // 相比ORB-SLAM2多了c3,c4
...
void Tracking::SearchLocalPoints()
{
    ...
    if(nToMatch>0)
    {
        ...
        //int matches = mspmatcher.SearchByProjection(mCurrentFrame, mvpLocalMapPoints, th);
        int matches = mspmatcher.SearchByProjection1(mCurrentFrame, mvpLocalMapPoints, th, mpLocalMapper->mbFarPoints, mpLocalMapper->mThFarPoints);
        //cout<<"mvpLocalMapPoints: "<<mvpLocalMapPoints.size()<<"  n to matches: "<<nToMatch<<"Matches: "<<matches<<endl;
    }
}
...
void Tracking::UpdateLocalKeyFrames()
{
    ...
    if(!mpAtlas->isImuInitialized() || (mCurrentFrame.mnId<mnLastRelocFrameId+2))
    {
        ...
        //cout<<"n:"<<n<<" mCurrentFrame.N = "<<mCurrentFrame.N<<endl;
    }
    else
    {
    ...
    //cout<<"local map number:"<<mvpLocalKeyFrames.size()<<endl;
...
cv::Mat Tracking::plotImages(Frame f1,Frame f2,std::vector<int> vmathcers12)
{
    /*
    Func:
   ...
   bool show = true;
    try
    {
        ...
        cv::Size2f figureSize(f1.imgLeft.cols*2, f1.imgLeft.rows);
        ...
        for (int i = 0; i < n; ++i) {
            ...
            //                 figureSize.height));
            cv::Rect roi(cv::Point(x_offset,0), cv::Size(320,240));
            cout<<"11113"<<endl;
...
cout<<"11114"<<endl;
        ...
        cout<<"11115"<<endl;
        plotMatches(figure , f1.mvKeys , f2.mvKeys ,vmathcers12);
        cout<<"11116"<<endl;
        for (const cv::KeyPoint& point : mInitialFrame.mvKeys) {
                cv::circle(mInitialFrame.imgLeft, point.pt, 2,cv::Scalar(0, 0, 255), 0.5);
            }
            for(const cv::KeyPoint& point : mCurrentFrame.mvKeys) {
                cv::circle(mCurrentFrame.imgLeft, point.pt, 2, cv::Scalar(0, 0, 255), 0.5);
            }
            cout<<"11117"<<endl;
            int total_width = mInitialFrame.imgLeft.cols + mCurrentFrame.imgLeft.cols;
            int max_height = std::max(mInitialFrame.imgLeft.rows, mCurrentFrame.imgLeft.rows);
            cv::Mat combined_image(max_height, total_width, mCurrentFrame.imgLeft.type());
            cout<<"11118"<<endl;
            mInitialFrame.imgLeft.copyTo(combined_image(cv::Rect(0, 0, mInitialFrame.imgLeft.cols, mInitialFrame.imgLeft.rows)));
            mCurrentFrame.imgLeft.copyTo(combined_image(cv::Rect(mCurrentFrame.imgLeft.cols, 0, mCurrentFrame.imgLeft.cols, mCurrentFrame.imgLeft.rows)));
            //cv::cvtColor(combined_image, combined_image, cv::COLOR_GRAY2BGR);
            cout<<"11118"<<endl;
            cv::Mat res;
            cv::vconcat(figure, combined_image, res);
            cout<<"11118"<<endl;
            cv::imshow("Feature Points", res);
            cv::waitKey(0);
...
void Tracking::plotMatches(const cv::Mat& figure , const std::vector<cv::KeyPoint>& kpts0, 
                const std::vector<cv::KeyPoint>& kpts1, const std::vector<int> vmatches12)
{
    float lw = 1; int ps = 2;
    int x_offset = 320;
    const cv::Scalar& color = cv::Scalar(0, 255, 0);
    std::vector<double> ratios = {1.0f , 1.0f};
    std::pair<std::vector<cv::Point2f>, std::vector<cv::Point2f>> kpts_pair;
    for(int i = 0 ; i < vmatches12.size(); i++)
    {
        if(vmatches12[i]!=-1)
        {
            kpts_pair.first.push_back(kpts0[i].pt);
            kpts_pair.second.push_back(kpts1[vmatches12[i]].pt);

        }
    }
    auto kpts_pt0 = kpts_pair.first;
    auto kpts_pt1 = kpts_pair.second;
    
    assert(kpts_pt0.size() == kpts_pt1.size());

    if (lw > 0)
    {
        for (unsigned int i = 0 ; i  < kpts_pt0.size() ; i++)
        {
            // 因为画布是计算比率缩放的，所以点也要进行缩放，同时右侧的图x坐标要加上左边的图的宽
            cv::Point2f pt0 = kpts_pt0[i];
            cv::Point2f pt1 = kpts_pt1[i];
            pt1.x += x_offset;
            ...
            cv::line(figure, pt0, pt1, color, 1 , cv::LINE_AA);
            if (ps > 0)
            {
