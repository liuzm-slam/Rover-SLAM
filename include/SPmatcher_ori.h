...
namespace ORB_SLAM3
{
    ...
    public:
        ...
        int SPmatcher::SearchBySP(Frame &CurrentFrame, const Frame &LastFrame);
        int SPmatcher::SearchByProjection(KeyFrame* pKF, Sophus::Sim3f &Scw, const vector<MapPoint*> &vpPoints,const std::vector<KeyFrame*> &vpPointsKFs,
        ...
        int MatchingPoints_onnx(Frame &f1, Frame &f2, vector<cv::Point2f> &vbPrevMatched, vector<int> &vnMatches12, bool outlier_rejection=false);
        int MatchingPoints(Frame &f1, Frame &f2, vector<cv::Point2f> &vbPrevMatched,vector<int> &vnMatches12, bool outlier_rejection=false);
        int MatchingPoints_onnx(std::vector<cv::Point2f> kpts0, const std::vector<cv::Point2f> kpts1, cv::Mat desc0,cv::Mat desc1, std::vector<int> vnMatches12);
